...
import { selectEmperor, count } from '../actions/index';
  ...
  renderList() {
    return this.props.emperors.map((emperor) => {
      return (
        <li
          key={emperor.name}
          onClick={() => {this.props.selectEmperor(emperor); this.props.count()}}
...
function mapDispatchToProps(dispatch) {
  ...
  return bindActionCreators({ selectEmperor: selectEmperor, count: count }, dispatch);
}
